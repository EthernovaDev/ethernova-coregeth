...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	// (miner symmetry — MUST match state_processor.go exactly)
	// ============================================================
	asyncRes := core.ProcessApplicationAsyncCallbacks(header, env.state, w.chain, w.chainConfig)
	if asyncRes != nil && !asyncRes.NoOp && (asyncRes.Dispatched > 0 || asyncRes.Reverted > 0 || asyncRes.Skipped > 0) {
		log.Debug("miner async callback dispatch",
