...
func geth(ctx *cli.Context) error {
	...
	oneClick := len(os.Args) == 1
	oneClickPort, err := applyEthernovaOneClickDefaults(ctx)
	if err != nil {
		return err
	}
	...
	if oneClick {
		fmt.Printf("Ethernova Node v%s (chain_id=%d network_id=%d)\n", params.Version, info.ChainID, info.NetworkID)
		if oneClickPort != 0 && oneClickPort != 30303 {
			fmt.Printf("p2p_port=%d (30303 in use)\n", oneClickPort)
		}
	}
