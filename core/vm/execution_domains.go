...
func (evm *EVM) executionDomainsActive() bool {
	if evm == nil || evm.chainConfig == nil || !ethernova.IsEthernovaChainID(evm.chainConfig.GetChainID()) {
		return false
	}
	if ethernova.ExecutionDomainForkBlock == 0 {
		return true
	}
	bn := evm.Context.BlockNumber
	return bn != nil && bn.Sign() >= 0 && bn.Uint64() >= ethernova.ExecutionDomainForkBlock
}

func (evm *EVM) domainOfAddress(addr common.Address) ExecutionDomain {
	if !evm.executionDomainsActive() {
		return DomainLegacy
	}
...
func (evm *EVM) checkContractDomainCall(caller ContractRef, callee common.Address) error {
	if !evm.executionDomainsActive() || !evm.callerIsContract(caller) {
		return nil
	}
