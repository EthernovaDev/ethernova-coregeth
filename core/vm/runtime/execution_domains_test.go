...
func postDomainForkConfig(sdb *state.StateDB) *Config {
	cfg := *params.AllEthashProtocolChanges
	cfg.ChainID = ethernova.NewChainIDBig
	return &Config{
		ChainConfig: &cfg,
		BlockNumber: new(big.Int).SetUint64(ethernova.ExecutionDomainForkBlock),
		State:       sdb,
	}
}

func preDomainForkConfig(sdb *state.StateDB) *Config {
	cfg := *params.AllEthashProtocolChanges
	cfg.ChainID = ethernova.NewChainIDBig
	return &Config{
		ChainConfig: &cfg,
		BlockNumber: new(big.Int).SetUint64(ethernova.ExecutionDomainForkBlock - 1),
		State:       sdb,
	}
}
...
func TestDomain0CannotCallNovaPrecompile(t *testing.T) {
	ret, _, err := Execute(callStateWitnessReturnSuccess(), nil, postDomainForkConfig(nil))
	if err != nil {
...
func TestDomain1CanCallNovaPrecompile(t *testing.T) {
	ret, _, err := Execute(domainCode(0x01, callStateWitnessReturnSuccess()), nil, postDomainForkConfig(nil))
	if err != nil {
...
func TestEIP3541StillRejectsUnknownEFPrefix(t *testing.T) {
	_, _, _, err := Create(returnInitcode([]byte{0xEF, 0x03, byte(vm.STOP)}), postDomainForkConfig(nil))
	if !errors.Is(err, vm.ErrInvalidCode) {
		t.Fatalf("unknown 0xEF prefix create error = %v, want %v", err, vm.ErrInvalidCode)
	}
}

func TestExecutionDomainPrefixPreForkStaysLegacy(t *testing.T) {
	if _, _, err := Execute(domainCode(0x01, []byte{byte(vm.STOP)}), nil, preDomainForkConfig(nil)); err == nil {
		t.Fatal("EF01 bytecode executed pre-fork; want legacy invalid-opcode behavior")
	}
	_, _, _, err := Create(returnInitcode(domainCode(0x01, []byte{byte(vm.STOP)})), preDomainForkConfig(nil))
	if !errors.Is(err, vm.ErrInvalidCode) {
		t.Fatalf("EF01 runtime create pre-fork error = %v, want %v", err, vm.ErrInvalidCode)
	}
}
...
func TestCapabilitiesNarrowAcrossCall(t *testing.T) {
	...
	ret, _, err := Execute(caller, nil, postDomainForkConfig(sdb))
	if err != nil {
