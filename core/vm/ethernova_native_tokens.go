...
func (c *novaTokenManager) RequiredGas(input []byte) uint64 {
	...
	switch input[0] {
	case 0x01:
		return 500000 // createToken: HIGH cost to prevent spam (Gemini review)
	case 0x02:
		return 5000 // transfer: cheap
	case 0x03:
		return 1000 // balanceOf: read-only
	case 0x04:
		return 1000 // tokenInfo: read-only
	default:
		return 0
	}
}
...
func (c *novaTokenManager) RunStateful(evm *EVM, caller common.Address, input []byte, _ bool) ([]byte, error) {
	if len(input) < 1 {
