...
func deferredPostForkBlock() uint64 {
	return ethernova.DeferredExecForkBlock + 1
}
...
func TestDeferredProcessing_EmptyQueueNoOp(t *testing.T) {
	sdb := newTestState(t)
	res := ProcessDeferredEffects(testHeader(deferredPostForkBlock()), sdb)
	if res == nil {
		t.Fatal("nil result")
	}
	if !res.NoOp {
...
func TestDeferredProcessing_DrainsFIFO(t *testing.T) {
	...
	sourceBlock := ethernova.DeferredExecForkBlock

	// Enqueue 5 entries at the activation block with distinguishable payloads.
	enqueued := make([]uint64, 5)
	for i := 0; i < 5; i++ {
		payload := []byte(fmt.Sprintf("entry-%d", i))
		enqueued[i] = directEnqueue(sdb, types.EffectTypeNoop, caller, payload, sourceBlock)
	}
...
func TestDeferredProcessing_PreForkIsNoOp(t *testing.T) {
	...
	preFork := ethernova.DeferredExecForkBlock - 1
	directEnqueue(sdb, types.EffectTypePing, caller, []byte{0x01}, preFork)

	res := ProcessDeferredEffects(testHeader(preFork), sdb)
	if !res.NoOp {
		t.Errorf("expected no-op before DeferredExec fork, got %+v", res)
	}
	if res.Processed != 0 || res.FailedHandlers != 0 {
		t.Errorf("pre-fork drain counters should stay zero, got %+v", res)
	}
	if got := vm.DqGetPendingCount(sdb); got != 1 {
		t.Errorf("pre-fork queue should remain untouched; pending=%d want 1", got)
	}
}
...
func TestDeferredProcessing_PingHandlerIncrementsCounter(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		directEnqueue(sdb, types.EffectTypePing, caller, []byte{byte(i)}, sourceBlock)
	}
...
func TestDeferredProcessing_Determinism(t *testing.T) {
	...
	mkAndDrain := func() common.Hash {
		...
		sourceBlock := ethernova.DeferredExecForkBlock
		for i := 0; i < 10; i++ {
			directEnqueue(sdb, types.EffectTypePing, caller, []byte{byte(i), byte(i ^ 0xff)}, sourceBlock)
		}
		res := ProcessDeferredEffects(testHeader(deferredPostForkBlock()), sdb)
		if res.Processed != 10 {
