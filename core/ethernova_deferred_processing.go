...
func dispatchDeferredEffect(entry *types.DeferredEffect, statedb *state.StateDB, blockNum uint64) error {
	...
	case types.EffectTypeAsyncCallback:
		// NIP-0004 Phase 11: AsyncCallback effects ARE enqueued by the
		// 0x30 register selector for `nova_getDeferredStats` visibility,
		// but the actual chain-driven dispatch is handled by
		// core.ProcessApplicationAsyncCallbacks via a per-target-block
		// index (see vm/ethernova_async_callbacks.go). Drain here is a
		// best-effort acknowledgement: we accept the entry and let it
		// clear from the queue. Real execution at the target block is
		// independent of this drain pass and runs AFTER deferred
		// processing in the same Phase 0 prologue.
		return nil
	case types.EffectTypeSessionUpdate:
		// Reserved for Phase 7 (Session). Accepted at enqueue, no-op at
		// drain until the corresponding fork phase lands a real handler.
		return nil
	default:
